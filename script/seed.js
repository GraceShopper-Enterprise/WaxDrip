...
const {
  db,
  models: { User, Emotion, OrderEmotion },
} = require("../server/db");
...
async function seed() {
  ...
  function eltUsers(numCalls){
    let arrOfPromises = []
  
    for(let i = 1; i <= numCalls; i++){
      let retFromApi1 = axios.get("https://randomuser.me/api/").then(({data}) => {
        let {name, email, login, location} = data.results[0]
  
        return User.create({username: `${name.first} ${name.last}`, password: login.password, email, type: new Date().getTime() % 2 === 0 ? "customer" : "siteAdmin", address: `${location.street.number} ${location.street.name}`})
      })

      arrOfPromises.push(retFromApi1)
    }
    ...
    // Creating Users
    const users = await Promise.all(eltUsers(3));
  ...
  const orders = await Promise.all([
    OrderEmotion.create({
      ...
      emotionId: 1,
      emotionQuantity: 1,
      emotionPriceAtPurchase: 1.0,
    }),
    OrderEmotion.create({
      orderId: 2,
      emotionId: 2,
      emotionQuantity: 2,
      emotionPriceAtPurchase: 2.0,
    }),
    OrderEmotion.create({
      orderId: 3,
      emotionId: 3,
      emotionQuantity: 3,
      emotionPriceAtPurchase: 3.0,
    }),
    OrderEmotion.create({
      orderId: 4,
      emotionId: 4,
      emotionQuantity: 4,
      emotionPriceAtPurchase: 4.0,
    }),
    OrderEmotion.create({
      orderId: 5,
      emotionId: 5,
      emotionQuantity: 5,
      emotionPriceAtPurchase: 5.0,
    }),
  ]);

  console.log(
    `seeded ${users.length}, users ${emotions.length} emotions, ${orders.length} orderEmotions`
  );
